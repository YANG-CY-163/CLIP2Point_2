...
trans_1 = transforms.Compose(
            [
                ...
                data_util.PointcloudNormalize(),
                data_util.PointcloudScale(lo=0.5, hi=2, p=1),
                data_util.PointcloudRotate(),
                data_util.PointcloudTranslate(0.5, p=1),
                data_util.PointcloudJitter(p=1),
                data_util.PointcloudRandomInputDropout(p=1),
            ])
...
class ShapeNetRender(ShapeNet):
    def __init__(self, partition='train', whole=False,aug = False):
        super().__init__(partition, whole)
    ...
    def __getitem__(self, idx):
        ...
        ## augmentation
        if self.aug:
            point_t1 = trans_1(points)
            point_t2 = trans_2(points)
            points = (point_t1,point_t2)
        ##
        else:
            points = torch.from_numpy(points).float()  #toTensor
