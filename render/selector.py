...
class ViewSelector(nn.Module):
    def __init__(self, nb_views, canonical_distance=1., transform_distance=False, input_view_noise=0.0):
        ...
        if self.nb_views == 10:
            views_elev = torch.tensor((0, 90, 180, 270, 225, 225, 315, 315, 0, 0), dtype=torch.float, requires_grad=False)
            views_azim = torch.tensor((0, 0, 0, 0, -45, 45, -45, 45, -90, 90), dtype=torch.float, requires_grad=False)
        elif self.nb_views == 6:
            views_elev = torch.tensor((0, 0, 0, 0, 90, -90), dtype=torch.float, requires_grad=False)
            views_azim = torch.tensor((0, 90, 180, 270, 0, 180), dtype=torch.float, requires_grad=False)
